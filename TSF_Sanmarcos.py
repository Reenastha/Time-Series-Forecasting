# -*- coding: utf-8 -*-
"""
Created on Thu Apr 27 15:11:10 2023
...
os.chdir(path)
df = pd.read_csv('Sanmarcos_KL.csv')

# Change the index to date format
df.index = pd.to_datetime(df['date'], format='%Y-%m-%d')

# Visualize the data
df["discharge"].plot(figsize=(16,8) , color='blue')
plt.title("Discharge(Sanmarcos)")
...
plt.title('Discharge (Sanmarcos)' , fontsize=18)
plt.xlabel('Date', fontsize=18)
plt.ylabel('Discharge' ,fontsize=18)
plt.plot(train['discharge'],linewidth=3)
plt.plot(test['discharge'],linewidth=3)
